...
public class IjkPlayerView extends FrameLayout implements View.OnClickListener {
    ...
    // 截屏建
    private ImageView mIvPlayerPhont;
    // 还原屏幕
    ...
    private void _initView(Context context) {
        ...
        mIvPlayerPhont.setOnClickListener(this);
    ...
    private void _initMediaPlayer() {
        ...
        if (!islive) {
            ...
            mPlayerSeek.setVisibility(INVISIBLE);
            mTvCurTime.setVisibility(INVISIBLE);
            mTvEndTime.setVisibility(INVISIBLE);
        }
        if (isback) {
            mIvBackWindow.setVisibility(GONE);
        }
        // 视频监听
    ...
    public void onResume() {
        ...
        if (mCurPosition != INVALID_VALUE && !islive) {
     ...
     * @return
     */
    public IjkPlayerView init(boolean islive, boolean isback) {
        this.islive = islive;
        this.isback = isback;
        _initMediaPlayer();
        return this;
...
//                start();
    ...
    public void reset(boolean b) {
        if (mIsEnableDanmaku && mDanmakuView != null&&!b) {
...
//        mPlayerThumb.setVisibility(View.GONE);
        ...
        if (mDialogClickListener != null) {
            mIvPlayerPhont.setVisibility(GONE);
        }
    ...
    private void _setControlBarVisible(boolean isShowBar) {
        if (mIsNeverPlay) {
            ...
            if (mIsFullscreen) {
                ...
                mIvPlayerLock.setVisibility(isShowBar ? View.VISIBLE : View.GONE);
                if (mDialogClickListener != null) {
                    mIvPlayerPhont.setVisibility(isShowBar ? View.VISIBLE : View.GONE);
                }
                if (mIsEnableDanmaku) {
                    if(islive){
    ...
    public void onClick(View v) {
        ...
        } else if (id == R.id.iv_player_phont) {
            _doScreenshot();
...
//            mDanmakuContext.setDanmakuSync(new VideoDanmakuSync(this));
    ...
    public boolean recoverFromEditVideo() {
        ...
        if (mIsFullscreen && mIsEnableDanmaku) {
            _recoverScreen();
        }
    ...
    private void _toggleDanmakuView(boolean isShow) {
        if (mIsEnableDanmaku) {
            if (isShow) {
                ...
                if(islive){
                    mTvTimeSeparator.setVisibility(INVISIBLE);
                }else {
    ...
    // 对话框点击监听，外部设置mDialogClickListener时截图按钮才会出现
