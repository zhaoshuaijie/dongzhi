...
public class MyOkHttp {
    ...
    public void upload(String url, Map<String, File> files, final IResponseHandler responseHandler) {
        upload(null, url, null, files,null,  responseHandler);
    }
    ...
    public void upload(String url, Map<String, String> params, Map<String, File> files, final IResponseHandler responseHandler) {
        upload(null, url, params, files, null, responseHandler);
    }
    ...
    public void upload(Context context, String url, Map<String, File> files, final IResponseHandler responseHandler) {
        upload(context, url, null, files,null,  responseHandler);
    }
    ...
    public void upload(Context context, String url, Map<String, String> params, Map<String, File> files, final IResponseHandler responseHandler) {
        upload(context, url, params, files,null,  responseHandler);
    }

    public void upload(Context context, String url, Map<String, String> params, Map<String, File> files, Map<String, ArrayList<File>> filess, final IResponseHandler responseHandler){
        ...
        if (filess != null && !filess.isEmpty()) {
            RequestBody fileBody;
            for (String key : filess.keySet()) {
                List<File> file = filess.get(key);
                for (int i = 0; i < file.size(); i++) {
                    String fileName = file.get(i).getName();
                    fileBody = RequestBody.create(MediaType.parse(guessMimeType(fileName)), file.get(i));
                    multipartBuilder.addPart(Headers.of("Content-Disposition",
                            "form-data; name=\"" + key + "\"; filename=\"" + fileName + "\""),
                            fileBody);
                }
